...
export default function CartBar({ onOpen }: CartBarProps) {
  ...
  const L = language === "zh"
    ? { item: "項", items: "項", sent: "已送", unsent: "未送" }
    : { item: "item", items: "items", sent: "sent", unsent: "unsent" };
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium">
            {cartCount} {cartCount === 1 ? L.item : L.items}
          </span>
          {sentCount > 0 && unsentCount > 0 ? (
            <span className="flex items-center gap-1.5">
              <span className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-full bg-green-50 border border-green-200">
                ...
                <span className="text-[10px] font-medium text-green-600 leading-none">{sentCount} {L.sent}</span>
              </span>
              <span className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-full bg-amber-50 border border-amber-200">
                <span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse" />
                <span className="text-[10px] font-medium text-amber-600 leading-none">{unsentCount} {L.unsent}</span>
              </span>
            </span>
          ) : sentCount > 0 ? (
            <span className="flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-green-50 border border-green-200">
              <span className="w-1.5 h-1.5 rounded-full bg-green-500" />
              <span className="text-[10px] font-medium text-green-600 leading-none">{L.sent}</span>
            </span>
          ) : null}
        </div>
